...
function StepTab({ step, isActive, onClick, isCompleted }: StepTabProps) {
  return (
    <motion.button
      ...
      onClick={onClick}
      className={cn(
        "flex items-center gap-2 rounded-xl px-2 py-1 text-left transition-colors w-full truncate",
        isActive ? "bg-white border border-border" : "hover:bg-muted/70 text-muted-foreground",
        "relative"
      )}
    ...
    >
      ...
      <div className="ml-auto">
        <CheckIcon className="w-5 h-5 text-white bg-green-400 rounded-full p-1" />
      </div>
    </motion.button>
  )
}
  ...
  if (isDesktop) {
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          className="!max-w-3xl p-4 gap-0 overflow-hidden bg-muted"
        ...
        >
          {/* <DialogHeader className="p-6 space-y-2 bg-muted border-b border-border">
            ...
            <DialogDescription>
              Follow the steps below to get started with the feature.
            </DialogDescription>
          </DialogHeader> */}

          <div className="h-full flex">
               <div className="flex flex-col flex-1 w-1/3 h-full p-2 pr-[18px]">
            ...
            <AnimatePresence mode="wait" initial={false}>
              <div className="w-2/3">
                <StepPreview
                  key={currentStep}
                  step={steps[currentStep] || { title: "" } as Step}
                  direction={direction}
                />
              </div>
            </AnimatePresence>
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-4 space-y-4 pb-32">
              ...
              <div className="relative aspect-[16/9] ring-2 ring-border ring-offset-8 ring-offset-background rounded-lg overflow-hidden">
                <StepPreview step={steps[currentStep] || { title: "" } as Step} direction={direction} />
              </div>
